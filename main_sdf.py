...
if __name__ == '__main__':
    ...
    if opt.test:
        ...
        from sdf.provider import SDFDataset, SDFLoader, UpdateDataset
        from loss import mape_loss

        grid_size = 64

        train_dataset = SDFDataset(opt.path, size=100, grid_size=grid_size, num_samples=2**14,
                                    sample_boxes=[
        ...
        train_loader = SDFLoader(train_dataset, batch_size=1, shuffle=True)

        update_dataset = UpdateDataset(opt.path, grid_size=grid_size)
        update_loader = torch.utils.data.DataLoader(update_dataset, batch_size=1)
        ...
        trainer = Trainer('ngp', model, workspace=opt.workspace, optimizer=optimizer, criterion=criterion, ema_decay=0.95, fp16=opt.fp16, lr_scheduler=scheduler, use_checkpoint='latest', eval_interval=10)

        trainer.train(train_loader, valid_loader, update_loader, 100000)
